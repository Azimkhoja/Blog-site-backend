...
module.exports = async function (req, res, next) {
  try {
    ...
    if (!token) {
      return res.send({ status: 401, message: "you should logged in" });
    }
    const {duty, id } = await jwt.verifyAccess(token);
    ...
    // if(!duty){
    //     return res.send({ status: 403, message: "you are not allowed"})
    // }
    // if(duty != "admin") {
